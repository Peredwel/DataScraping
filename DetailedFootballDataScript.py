# Carolyn Sullivan, Oct. 28-29, 2019
...
base_url = "https://www.transfermarkt.com/spieler-statistik/wertvollstespieler/marktwertetop?ajax=yw1&altersklasse=alle&ausrichtung=alle&jahrgang=0&land_id=0&plus=1&spielerposition_id=alle"
columns = ['PlayerName', 'Position', 'Age', 'Nationality', 'Club', 'MarketValue', 'Matches', 'Goals', 'Owngoals', 'Assists', 'Yellowcards', 'SecondYellowCard', 'Redcard','SubstitutionsOn', 'SubstitutionsOff']
...
for offset in range(0,20): # Use range 20 as there are 20 pages of data.
    ...
    for row in playerRows:                # .findAll returns a list of table rows, iterate over these (cleaner to find once?)
        if count%3 == 0:                    #get correctly formatted row
            ...
            print(Owngoals)
            Assists = td[12].text
            Yellowcards = td[13].text
            SecondYellowCard = td[14].text
            Redcard = td[15].text
            SubstitutionsOn = td[16].text
            SubstitutionsOff = td[17].text

            player_data = pd.DataFrame([[PlayerName, Position, Age, Nationality, Club, MarketValue, Matches, Goals, Owngoals, Assists, Yellowcards, SecondYellowCard, Redcard, SubstitutionsOn, SubstitutionsOff]])   #Shove values in dataframe.
...
data.to_csv('detailedFootballData.csv', encoding='utf-8-sig')   #Place in Csv file
